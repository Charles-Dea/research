...
#     config.gradient_accumulation_steps = 8
    ...
    model_config.num_attention_heads = 2 # 8
    model_config.num_hidden_layers = 1 # 8
    model_config.hidden_size = 128 # 512
    model_config.intermediate_size = 256 # 2048
...
def get_pathfinder128_config():
    config=ml_collections.ConfigDict()
    config.batch_size=64
    config.tokenizer=make_gray_img_tokenizer(il=128)
    tots=200
    trns=int(tots*.85)
    tsts=tots-trns
    config.eval_frequency=trns//config.batch_size
...
def get_pathfinder256_config():
    config=ml_collections.ConfigDict()
    config.batch_size=64
    config.tokenizer=make_gray_img_tokenizer(il=256)
    tots=200
    trns=int(tots*.85)
    tsts=tots-trns
    config.eval_frequency=trns//config.batch_size
